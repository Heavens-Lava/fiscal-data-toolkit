...
export async function stock(ticker) {
  ...
  const [revR, gpR, niR, ocfR, eqR] = await Promise.all([
  ...
  const tRev = ttmSum(revR), tNi = ttmSum(niR), tOcf = ttmSum(ocfR);
  ...
  return {
    ...
    ttm: { revenue: tRev, netIncome: tNi, ocf: tOcf, netMargin: tRev && tNi != null ? tNi / tRev : null },
    price, marketCap: mcap, equity: latestInstant(eqR),
