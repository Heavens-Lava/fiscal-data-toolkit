...
// ── Markets & interest rates (FRED) ──────────────────────────────────────────
// Annual average of a (daily/monthly/weekly) series -> { year: avg }.
const annualAvg = (a) => {
  const g = {};
  for (const x of a) (g[x.d.slice(0, 4)] ||= []).push(x.v);
  return Object.fromEntries(Object.entries(g).map(([y, vs]) => [y, vs.reduce((s, v) => s + v, 0) / vs.length]));
};

export async function markets() {
  const [sp, ten, mort, ff] = await Promise.all([
    fred("SP500"), fred("DGS10"), fred("MORTGAGE30US"), fred("FEDFUNDS"),
  ]);
  const ffY = annualAvg(ff), tenY = annualAvg(ten), mortY = annualAvg(mort);
  const years = Object.keys(ffY).map(Number).sort().slice(-15);
  const rateHistory = years.map((y) => ({
    year: y, fedFunds: ffY[y] ?? null, tenYear: tenY[y] ?? null, mortgage: mortY[String(y)] ?? null,
  }));
  return {
    ...
    fedFunds: last(ff).v, fedFundsDate: last(ff).d, fedFundsHistory: ff.slice(-30).map((x) => x.v),
    rateHistory,
  };
}

// ── Housing market (FRED) ────────────────────────────────────────────────────
export async function housing() {
  const [price, csi, starts, mort] = await Promise.all([
    fred("MSPUS"),        // median sales price of houses sold ($, quarterly)
    fred("CSUSHPISA"),    // Case-Shiller U.S. national home price index (monthly)
    fred("HOUST"),        // housing starts (thousands of units, annual rate, monthly)
    fred("MORTGAGE30US"), // 30-yr mortgage rate
  ]);
  const csY = csi.slice(-13); // ~1yr for YoY
  const yoy = csY.length >= 13 ? (last(csi).v / csY[0].v - 1) * 100 : null;
  return {
    medianPrice: last(price).v, medianPriceDate: last(price).d, priceHistory: price.slice(-24).map((x) => x.v),
    caseShiller: last(csi).v, caseShillerYoY: yoy, caseShillerHistory: csi.slice(-36).map((x) => x.v),
    housingStarts: last(starts).v, startsDate: last(starts).d,
    mortgage30: last(mort).v,
  };
}
